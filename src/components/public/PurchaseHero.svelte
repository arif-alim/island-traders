<div class="bg-gray-900">
	<div class="grid grid-cols-1 sm:grid-cols-12 mx-auto max-w-8xl">
		<div class="col-span-1 flex sm:col-span-6 w-5/6 items-center">
			<div class="py-12 sm:py-4 px-4 sm:px-4">
				<div class="flex-col">
					<h1 class="text-3xl  font-extrabold text-white sm:text-4xl">Purchases</h1>
					<h2 class="mt-3 text-lg text-gray-300 ">
						If you need help purchasing items, we can help!
					</h2>
				</div>
			</div>
		</div>

		<div class="hidden sm:flex col-span-1 sm:col-span-6 h-[350px] ">
			<div class="relative">
				<svg
					class="block absolute right-0 inset-y-0 h-full w-48 text-white transform translate-x-1/2"
					fill="currentColor"
					viewBox="0 0 100 100"
					preserveAspectRatio="none"
					aria-hidden="true"
				>
					<polygon points="50,0 100,0 50,100 0,100" />
				</svg>
			</div>
			<div class="w-full">
				<img
					class="object-cover h-full w-full"
					src="/images/public/services/purchase/help-purchase.jpg"
					alt=""
				/>
			</div>
		</div>
	</div>
</div>
